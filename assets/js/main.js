	...
	$.ajax({
		...
		success: res => {
			...
			if (Object.keys(booking).length) {
				...
				$('[name="bok_advance_amount"]', parent).val(booking.bok_advance_amount);
				...
				if (inStock) {
						...
						<div class="row g-3">
							<div class="col-6">
								...
								<div class="input-group input-group-sm">
									...
									<input type="text" name="bill_advance_date[]" class="js-flatpickr form-control form-control-sm" required value="${new Date(bok.bok_created).getFormated('yyyy-mm-dd')}">
								</div>
							</div>
							<div class="col-6">
								...
								<div class="input-group input-group-sm">
									...
									<input type="text" name="bill_advance_amount[]" class="form-control rounded-end-1 px-2" oninput="allowType(event, 'decimal', 2),calcBillTotal()" value="${Number(bok.bok_advance_amount).toFixed(2)}" required onfocus="this.select()">
					...
					products.stock.forEach(bok_item => {
						bok_item.stock.forEach(pro => {
							...
							$('[name="bds_pro_id[]"]', billing_product[count]).find(`option[pro-stock-id="${pro.st_id}"]`).attr('selected', true);
...
function addBillAdvance(el) {
	let callback = el.closest('#receiveBillAmount') ? 'calcReceiveBillTotal();' : 'calcBillTotal();';
	const advance = $(`<div class="col-12 bill-advance-item">
		<div class="row g-3">
			<div class="col-6">
				<label class="form-label">${translate('received_date')}</label>
...
function receiveBillAmount(bill_id) {
	$.ajax({
		...
		success: res => {
					...
					<div class="row g-3">
						<div class="col-6">
							...
							<div class="input-group input-group-sm">
								...
								<input type="text" name="bill_advance_date[]" class="js-flatpickr form-control form-control-sm" required value="${Object.keys(ad)[0]}">
							</div>
						</div>
						<div class="col-6">
							<label class="form-label">${translate('amount')} <span class="rupee"></span></label>
							<div class="input-group input-group-sm">
								...
								<input type="text" name="bill_advance_amount[]" class="form-control rounded-end-1 px-2" oninput="allowType(event, 'decimal', 2),calcReceiveBillTotal();" value="${Number(Object.values(ad)[0]).toFixed(2)}" required onfocus="this.select()">
								<button class="btn border-0 p-0" type="button" onclick="removeBillAdvance(this)">
									<i class="bi bi-x-circle-fill p-1 ms-1 text-danger"></i>
								</button>
							</div>
			...
			$('#receive-bill-balance', parent).val((Number(bill.bill_total) - Number(bill.bill_paid)).toFixed(2));
			parent.modal('show');
		}
	});
}
...
function viewBusiness(biz_id) {
	...
	if (Object.keys(biz).length) {
		...
		$('.modal-body .row', parent).html(`<div class="col-12 col-md-3">
					...
					<div class="col-12 col-md-9">
						<button class="btn btn-sm btn-outline-danger py-1" onclick="clearBizData(${biz.biz_id})">${translate('clear')}</button>
					</div>`);
		parent.modal('show');
	}
}
